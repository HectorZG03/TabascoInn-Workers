...
class SubirDocumentoModal {
    constructor(trabajadorId) {
        ...
        this.modal = null; // Referencia al modal de Bootstrap
    ...
    bindEvents() {
        // Modal events usando API nativa de Bootstrap
        const modalElement = document.getElementById('subirDocumentoModal');
        if (modalElement) {
            modalElement.addEventListener('show.bs.modal', () => this.initModal());
            modalElement.addEventListener('hidden.bs.modal', () => this.resetForm());
        }
        
        // Mantener jQuery para el formulario y otros elementos
    ...
    open() {
         ...
         */
        isOpen() {
            return this.modal ? this.modal._isShown : false;
        }
    /**
