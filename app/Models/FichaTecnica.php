   ...
   public function calcularFechaFinVacaciones($fechaInicio, $diasSolicitados)
    {
        ...
        // ✅ CAMBIO PRINCIPAL: Calcular fecha fin excluyendo DESCANSOS Y FESTIVOS
        $fechaActual = Carbon::parse($fechaInicio);
        $diasContados = 0;
        $maxIteraciones = 365; // Prevenir loops infinitos
        $iteraciones = 0;

        // Si la fecha inicio no es día hábil, avanzar al siguiente día hábil
        while (!$this->esDiaHabilCompleto($fechaActual, $diasLaborablesNumeros) && $iteraciones < $maxIteraciones) {
            $fechaActual->addDay();
            $iteraciones++;
        }

        // Contar días hábiles hasta completar los días solicitados
        while ($diasContados < $diasSolicitados && $iteraciones < $maxIteraciones) {
            // ❌ LÍNEA ANTERIOR (solo verificaba días laborables):
            // if (in_array($fechaActual->dayOfWeek, $diasLaborablesNumeros)) {
            
            // ✅ NUEVA LÍNEA (verifica laborables Y no festivos):
            if ($this->esDiaHabilCompleto($fechaActual, $diasLaborablesNumeros)) {
                $diasContados++;
            }
    ...
    private function esDiaHabilCompleto($fecha, $diasLaborablesNumeros)
    {
        $fechaCarbon = Carbon::parse($fecha);
        
        // 1. Debe ser día laborable para el trabajador
        $esDiaLaborable = in_array($fechaCarbon->dayOfWeek, $diasLaborablesNumeros);
        
        // 2. NO debe ser día festivo oficial
        $noEsFestivo = !\App\Models\DiaFestivo::esDiaFestivo($fechaCarbon);
        
        return $esDiaLaborable && $noEsFestivo;
    }
