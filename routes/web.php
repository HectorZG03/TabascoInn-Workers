...
use App\Http\Controllers\GestionUsuariosAdminController;
...
Route::middleware(['auth'])->group(function () {
    ...
    // ✅ RUTAS DE GESTIÓN DE USUARIOS ADMINISTRATIVOS (Solo para SuperAdmin)
    Route::prefix('usuarios-admin')->name('usuarios.admin.')->group(function () {
        Route::get('/', [GestionUsuariosAdminController::class, 'listaUsuariosAdmin'])->name('lista');
        Route::get('/crear', [GestionUsuariosAdminController::class, 'formularioCrearAdmin'])->name('crear');
        Route::post('/crear', [GestionUsuariosAdminController::class, 'crearUsuarioAdmin'])->name('guardar');
        Route::get('/{id}/editar', [GestionUsuariosAdminController::class, 'formularioEditarAdmin'])->name('editar');
        Route::put('/{id}', [GestionUsuariosAdminController::class, 'actualizarUsuarioAdmin'])->name('actualizar');
        Route::patch('/{id}/estado', [GestionUsuariosAdminController::class, 'cambiarEstadoAdmin'])->name('cambiar-estado');
        Route::delete('/{id}', [GestionUsuariosAdminController::class, 'eliminarUsuarioAdmin'])->name('eliminar');
    });
    ...
    Route::prefix('configuracion')->group(function () {
      ...
      Route::prefix('dias-festivos')->name('configuracion.dias_festivos.')
            ->middleware('check.permiso:dias_festivos')->group(function () {
                Route::get('/', [DiasFestivosController::class, 'index'])->name('index');
                
                Route::middleware('check.permiso:dias_festivos,crear')->group(function () {
                    Route::post('/', [DiasFestivosController::class, 'store'])->name('store');
                    Route::post('/generar', [DiasFestivosController::class, 'generarOficiales'])->name('generar');
                    Route::post('/copiar', [DiasFestivosController::class, 'copiarAño'])->name('copiar');
                });
                
                Route::middleware('check.permiso:dias_festivos,editar')->group(function () {
                    Route::put('/{diaFestivo}', [DiasFestivosController::class, 'update'])->name('update');
                });
                
                Route::middleware('check.permiso:dias_festivos,eliminar')->group(function () {
                    Route::delete('/{diaFestivo}', [DiasFestivosController::class, 'destroy'])->name('destroy');
                });
                
                // APIs
                Route::get('/api/por-año', [DiasFestivosController::class, 'obtenerPorAño'])->name('api.por-año');
                Route::post('/api/calcular-reintegro', [DiasFestivosController::class, 'calcularFechaReintegro'])->name('api.calcular-reintegro');
        });
    });
    ...
    Route::prefix('trabajadores')->name('trabajadores.')->group(function () {
        ...
        Route::prefix('{trabajador}/perfil')->name('perfil.')->middleware('check.permiso:trabajadores,editar')->group(function () {
            ...
            Route::put('/contacto-emergencia', [ActPerfilTrabajadorController::class, 'updateContactoEmergencia']) ->middleware('check.permiso:trabajadores,editar') ->name('update-contacto');
        ...
        Route::prefix('{trabajador}/horas-extra')->name('horas-extra.')
            ...
            ->middleware('check.permiso:horas_extra')->group(function () {
                ...
                Route::middleware('check.permiso:horas_extra,editar')->group(function () {
                    ...
                    Route::get('{horaExtra}/editar', 'edit')->name('editar');
                    Route::put('{horaExtra}', 'update')->name('actualizar');
                    Route::post('{horaExtra}/subir-documento', 'subirDocumento')->name('subir-documento');
                });
                
                Route::get('{horaExtra}/documento/{documento}/descargar', 'descargarDocumento')->name('descargar-documento');
        });
        ...
        Route::prefix('{trabajador}/vacaciones')->name('vacaciones.')
            ->middleware('check.permiso:vacaciones')->group(function () {
                ...
                // En la sección de vacaciones, agregar:
                Route::post('/calcular-reintegro', [VacacionesController::class, 'calcularFechaReintegro'])->name('calcular-reintegro');
                ...
                // En web.php - dentro del grupo de rutas de vacaciones
                Route::middleware('check.permiso:vacaciones,eliminar')->group(function () {
                    Route::delete('/{vacacion}/eliminar', [VacacionesController::class, 'eliminar'])->name('eliminar');
                });
        });
        ...
        Route::prefix('{trabajador}/documentos-vacaciones')->name('documentos-vacaciones.')
            ->middleware('check.permiso:vacaciones')->group(function () {
                ...
                // En la sección de documentos de vacaciones, actualizar esta ruta:
        ...
        Route::get('/trabajadores-vacaciones', [VacacionesController::class, 'listarTrabajadoresConVacaciones'])
        ->middleware('check.permiso:vacaciones')
        ->name('trabajadores.vacaciones.lista');
    ...
    Route::get('/notificaciones/eventos', [NotificationController::class, 'eventosProximos'])
    ->middleware('auth')
